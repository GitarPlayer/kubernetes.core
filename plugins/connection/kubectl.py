...
class Connection(ConnectionBase):
    ...
    def _build_exec_cmd(self, cmd):
        ...
        # These options come after 'kubectl exec'
        exec_options = []
        censored_exec_options = []
        ...
        for key in doc_yaml.get("options"):
            if key.endswith("verify_ssl") and self.get_option(key) != "":
                ...
                exec_options.append("{0}={1}".format(self.connection_options[key], str(skip_verify_ssl).lower()))
                censored_exec_options.append("{0}={1}".format(self.connection_options[key], str(skip_verify_ssl).lower()))
                ...
                exec_options += [cmd_arg, kubeconfig_path]
                censored_exec_options += [cmd_arg, kubeconfig_path]
            elif (
                ...
                cmd_arg = self.connection_options[key]
                exec_options += [cmd_arg, self.get_option(key)]
                # Redact password and token from console log
                if key.endswith(("_token", "_password")):
                    censored_exec_options += [cmd_arg, "********"]
                else:
                    censored_exec_options += [cmd_arg, self.get_option(key)]
    
        extra_args_name = "{0}_extra_args".format(self.transport)
        if self.get_option(extra_args_name):
            exec_options += self.get_option(extra_args_name).split(" ")
            censored_exec_options += self.get_option(extra_args_name).split(" ")
